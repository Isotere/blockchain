...
func main() {
	...
	if errInner := run(log); errInner != nil {
		log.Errorw("startup", "ERROR", errInner)
		_ = log.Sync()
		os.Exit(1)
	}
}
...
func run(log *zap.SugaredLogger) error {
	...
	go func() {
		if errInner := http.ListenAndServe(cfg.Web.DebugHost, debugMux); errInner != nil {
			log.Errorw("shutdown", "status", "debug v1 router closed", "host", cfg.Web.DebugHost, "ERROR", errInner)
		}
	}()
